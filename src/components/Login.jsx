    ...
    <div style={styles.bg}>
      <div style={styles.card}>
        ...
        <div style={styles.tabs}>
          <button style={view === 'login' ? styles.tabActive : styles.tab} onClick={() => { setView('login'); setMensaje(''); }}>
            Ingresar
          </button>
          <button style={view === 'registro' ? styles.tabActive : styles.tab} onClick={() => { setView('registro'); setMensaje(''); }}>
            Registrarse
          </button>
        </div>
        ...
        {view === 'login' && (
          <form onSubmit={handleLogin} style={styles.form}>
            ...
            <input name="password" type="password" placeholder="••••••••" value={formData.password} onChange={handleChange} required style={styles.input} />
            <div style={styles.olvidaste}>
              <button type="button" style={styles.linkBtn} onClick={() => { setView('recuperar'); setMensaje(''); }}>
        ...
        {view === 'registro' && (
          <form onSubmit={handleRegistro} style={styles.form}>
            ...
            <label style={styles.label}>Contraseña</label>
            <input name="password" type="password" placeholder="Mínimo 6 caracteres" value={formData.password} onChange={handleChange} required style={styles.input} />
            ...
            {mensaje && <p style={styles.mensaje}>{mensaje}</p>}
            <button type="submit" style={styles.btnPrimary} disabled={loadingReg}>
        ...
        {view === 'recuperar' && (
          <form onSubmit={handleRecuperar} style={styles.form}>
            ...
            <button type="button" style={styles.btnSecondary} onClick={() => { setView('login'); setMensaje(''); }}>
              Volver al login
            </button>
          </form>
        )}
...
const styles = {
  ...
  tab: {
    ...
    background: 'transparent', color: '#666', fontWeight: '500', cursor: 'pointer', fontSize: '14px',
  },
  tabActive: {
    flex: 1, padding: '10px', border: 'none', borderRadius: '8px',
    background: '#ff6b1a', color: '#fff', fontWeight: '600', cursor: 'pointer', fontSize: '14px',
  },
  ...
  input: {
    ...
    padding: '12px 14px', color: '#fff', fontSize: '15px', outline: 'none',
    width: '100%', boxSizing: 'border-box',
  },
  rolWrap: { display: 'flex', gap: '10px', marginTop: '4px' },
  rolBtn: {
    flex: 1, background: '#111', border: '1px solid #2a2a2a', borderRadius: '10px',
    padding: '14px 10px', cursor: 'pointer', display: 'flex', flexDirection: 'column',
    alignItems: 'center', gap: '4px',
  },
  rolBtnActive: {
    flex: 1, background: '#1f1208', border: '2px solid #ff6b1a', borderRadius: '10px',
    padding: '14px 10px', cursor: 'pointer', display: 'flex', flexDirection: 'column',
    alignItems: 'center', gap: '4px',
  },
  ...
  btnPrimary: {
    marginTop: '20px', background: '#ff6b1a', color: '#fff', border: 'none',
    borderRadius: '8px', padding: '14px', fontSize: '16px', fontWeight: '600',
    cursor: 'pointer', width: '100%',
  },
  btnSecondary: {
    marginTop: '8px', background: 'transparent', color: '#666', border: '1px solid #333',
    borderRadius: '8px', padding: '12px', fontSize: '14px', cursor: 'pointer', width: '100%',
  },
