...
function Home({ solicitudes, user, onChangeView, onLogout, onCotizar, currentView }) {
  ...
  const [vistaActiva, setVistaActiva] = useState('solicitudes'); // 'solicitudes' o 'mapa'
  ...
  return (
    <div style={styles.bg}>
      ...
      {modalSol && (
        <ModalCotizar sol={modalSol} onClose={() => setModalSol(null)} onSubmit={handleCotizarSubmit} />
      )}
      ...
      <div style={styles.navbar}>
        <button style={{ ...styles.navBtn, ...(currentView === 'home' ? styles.navBtnActive : {}) }} onClick={() => onChangeView('home')}>
          Explorar
        </button>
        <button style={{ ...styles.navBtn, ...(currentView === 'mis' ? styles.navBtnActive : {}) }} onClick={() => onChangeView('mis')}>
          Mis solicitudes
        </button>
        {esTecnico && (
          <button style={{ ...styles.navBtn, ...(currentView === 'trabajos' ? styles.navBtnActive : {}) }} onClick={() => onChangeView('trabajos')}>
            Mis trabajos
          </button>
        )}
      ...
      {/* Tabs solicitudes / mapa */}
      <div style={styles.tabsWrap}>
        <button
          style={{ ...styles.tabBtn, ...(vistaActiva === 'solicitudes' ? styles.tabBtnActive : {}) }}
          onClick={() => setVistaActiva('solicitudes')}
        >
          📋 Solicitudes
        </button>
        <button
          style={{ ...styles.tabBtn, ...(vistaActiva === 'mapa' ? styles.tabBtnActive : {}) }}
              ...
              <div style={styles.grid}>
                {solicitudesAbiertas.map((sol) => (
                  <div key={sol.id} style={styles.card}>
                    ...
                    {esTecnico ? (
                      <button style={styles.cotizarBtn} onClick={() => setModalSol(sol)}>
                        Enviar cotización
                      </button>
                    ) : (
...
const styles = {
  ...
  modalBody: { padding: '0 20px', display: 'flex', flexDirection: 'column', gap: '14px' },
